    ...
    <!-- adminKit : 끝 -->
		...
		<main class="content">
			<div class="container-fluid p-0">
				...
				<h1 class="h3 mb-3"><strong>객실후기</strong> 신고 관리</h1>
							...
							<table class="table table-hover my-0">
								...
								<tbody> 
									<c:forEach items="${reportList}" var="r" >
										<tr>
									        <td>${r.accrReportNo}</td>
									        <td>${r.accomReviewNo}</td>
									        <td>${r.memberId}</td>
             ...
             <div class="container">
                <ul class="nav justify-content-center bg-light">
                   <c:if test="${beginRow > (ROW_PER_PAGE * 10)}">
                      <li><a href="${pageContext.request.contextPath}/admin/accomReviewReportList?currentPage=${pageNo-1}">&lt;</a></li>
                   </c:if>
                   ...
                   <c:forEach var="f" begin="${pageNo}" end="${pageNo + 9}">
                      <c:if test="${not doneLoop}">
                         <c:choose>
                            ...
                            <c:otherwise>
                               <li><a class="nav-link active" href="${pageContext.request.contextPath}/admin/accomReviewReportList?currentPage=${f}">${f}</a></li>
                            </c:otherwise>
                         </c:choose>
                   ...
                   <c:if test="${currentPage + 10 <= lastPage}">
                      <li><a class="nav-link active" href="${pageContext.request.contextPath}/admin/accomReviewReportList?currentPage=${pageNo+10}">&gt;</a></li>
                   </c:if>
                </ul>
          </div> 
    <!-- adminFooter : 시작 -->

...
public class AdminController {
	 ...
	 * 
	 * @GetMapping("/admin/productManagement") public String management() {
	...
	public String getHostList(Model model, @RequestParam(defaultValue = "1") int currentPage) {
		System.out.println("AdminController()_host실행");
		log.debug("★★★[boryeong]AdminController★★★" + currentPage);

		int beginRow = (currentPage * ROW_PER_PAGE) - (ROW_PER_PAGE - 1);

		Map<String, Object> map = adminService.getHostList(currentPage, ROW_PER_PAGE);
		// 값
		model.addAttribute("beginRow", beginRow);
		model.addAttribute("ROW_PER_PAGE", ROW_PER_PAGE);
		model.addAttribute("hostList", map.get("hostList"));
		model.addAttribute("lastPage", map.get("lastPage"));
		model.addAttribute("currentPage", currentPage);
		int pageNo = ((beginRow / 100) * 10 + 1);
		log.debug("★★★[boryeong]AdminController_pageNo★★★" + pageNo);
